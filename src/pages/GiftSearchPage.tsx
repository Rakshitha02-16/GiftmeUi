...
const Tab1: React.FC = () => {
  ...
  const [keyword, setKeyword] = useState("1"); // Default keyword
  ...
  const searchGifts = async () => {
    setLoading(true);
    try {
      const data = await fetchGifts(keyword, 1, 10);
      setGifts(data);
    } catch (error) {
  ...
  useEffect(() => {
    searchGifts(); // Fetch gifts on component mount
  }, []);
  return (
    <IonPage>
        ...
        <IonToolbar>
          <IonItem>
            <IonToggle
              ...
              alignment="center"
              checked={appData.gender}
              onIonChange={(e) => updateData("gender", e.target.value)}
            >
              Female
            </IonToggle>
            Male
          ...
          <IonList>
            <IonItem>
              <IonSelect
                ...
                value={appData.relationship}
                onIonChange={(e) => updateData("relationship", e.target.value)}
              >
          ...
          <IonList>
            <IonItem>
              <IonSelect
                ...
                labelPlacement="floating"
                placeholder="Select an Option"
                value={appData.occasion}
                onIonChange={(e) => updateData("occasion", e.target.value)}
              >
                <IonSelectOption value="birthday">Birthday</IonSelectOption>
                <IonSelectOption value="anniversary">
                  Anniversary
                </IonSelectOption>
          ...
          <IonList>
            <IonItem>
              ...
              >
                ...
                <IonSelectOption value="perfumes">
                  Perfumes & Fragrances
                </IonSelectOption>
          ...
          <IonItem>
            <IonRange
              ...
              max={100000}
              pin={true}
              onIonChange={budgetHandleChange}
              value={appData.budget}
              onIonInput={(e) => updateData("budget", e.target.value)}
            >
              <IonText slot="start" color="primary">
                ${lowbudget}
              </IonText>
          ...
          <IonModal isOpen={isModalOpen} onDidDismiss={handleCloseModal}>
            <IonHeader style={{ padding: "14px" }}>
              ...
              <IonButton
                ...
                style={{ position: "relative", left: "8.9em" }}
              >
                <IonIcon
                  slot="icon-only"
                  icon={close}
                    ...
                    <IonItemSliding key={index}>
                      <IonItem button={true}>
                        ...
                        <div className="itemWrapper">
                          ...
                          <p style={{ marginTop: "10px" }}>{item.bagDesc}</p>
                        </div>
                        <div className="priceWrapper">
                          ₹{item.bagPrice.toFixed(1)}
                        </div>
                      </IonItem>
